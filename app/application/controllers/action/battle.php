...
class Battle extends CI_Controller
{
			...
			if ( $monster != null )
			{
				...
				while ( $attacker[ 'health' ] > 0 && $defender[ 'health' ] > 0 )
				{
					...
					else
					{
						$rand = rand( 0, 100000 ) / 100000;
						$damageResult = intval( 
								(($attacker[ 'atk' ] - $attacker[ 'atk_min' ]) * $rand + $attacker[ 'atk_min' ]) * $levelFix * (1 - $defender[ 'def_percent' ]) );
						$damage = array (
								'skill' => '',
								'damage' => $damageResult 
						);
				...
				if ( $win )
				{
					...
					{
						$restTime = ceil( 
								($role[ 'health_max' ] - $role[ 'health' ]) / $recover_health );
					}
	...
	private function getMonsterByNearestLevel()
	{
		...
		if ( !empty( $result ) )
		{
			$resultMonster = $this->mongo_db->where_in( 'id', $result[ 'monster' ] )->order_by( 
					array (
							'level' => 'desc' 
					) )->get( 'monster' );
