...
class Battle extends CI_Controller
{
        ...
        if ($this->currentRole->role['current_action'] == '1') {
            ...
            if ($this->currentRole->role ['health'] == '0') {
                ...
                if ($this->currentRole->role ['next_battletime'] > $time) {
                    ...
                    if ($this->currentRole->role ['health']
                        > $this->currentRole->role ['health_max']) {
                        $this->currentRole->role ['health']
                            = $this->currentRole->role ['health_max'];
                    }
                    ...
                    if ($monster != null) {
                        ...
                        $this->currentRole->role ['atk_min'] = $this->currentRole->role ['atk']
                                                               * $d;
                        $this->currentRole->role ['def_percent'] = $k
                                                                   * $this->currentRole->role ['def']
                                                                   / ($m + $k
                                                                           * $this->currentRole->role ['def']);
                        $this->currentRole->role ['mdef_percent'] = $k
                                                                    * $this->currentRole->role ['mdef']
                                                                    / ($m + $k
                                                                            * $this->currentRole->role ['mdef']);
                        $monster ['atk_min'] = $monster ['atk'] * $d;
                        $monster ['def_percent'] = $k * $monster ['def'] / ($m + $k
                                                                                 * $monster ['def']);
                        $monster ['mdef_percent'] = $k * $monster ['mdef'] / ($m + $k
                                                                                   * $monster ['mdef']);
                        ...
                        while ($attacker ['health'] > 0 && $defender ['health'] > 0) {
                            ...
                            if (!empty($damage)) {
                                if (isset($damage ['damage'])) {
                                    ...
                                    $defender ['health'] = $defender ['health'] < 0 ? 0
                                        : $defender ['health'];
                                }
                                $item ['round'] = $round;
                            ...
                            $defender ['def_percent'] = $k * $defender ['def'] / ($m + $k
                                                                                       * $defender ['def']);
                            $defender ['mdef_percent'] = $k * $defender ['mdef'] / ($m + $k
                                                                                         * $defender ['mdef']);
                            ...
                            if ($defender ['health'] <= 0) {
                                ...
                                if (isset($this->currentRole->role['status'])
                                    && is_array(
                                        $this->currentRole->role['status']
                                    )) {
                        ...
                        if ($win) {
                            ...
                            $judgmentHealth = $this->currentRole->role ['health_max']
                                              * $restHealthPercentage;
                            if ($this->currentRole->role ['health'] <= $judgmentHealth) {
                                $restTime = ceil(
                                    ($judgmentHealth - $this->currentRole->role ['health'])
                                    / $recover_health
                                );
                            }
                            $this->currentRole->role ['exp'] += $settle ['exp'];
                            $this->currentRole->role ['gold'] += $settle ['gold'];
                            if ($this->currentRole->role ['exp']
                                >= $this->currentRole->role ['nextexp']) {
                                // 升级
                                //Gift hook: 升级前
                        ...
                        $battleResult ['next_battletime']
                            = $this->currentRole->role ['next_battletime'];
    ...
    private function _settle_battle($monster, $role)
    {
        ...
        if ($rand <= ($monster['equipment_drop']) * $rate_inc) {
            if (!empty($monster['equipments']) && is_array($monster['equipments'])) {
                ...
                if (!empty($id)) {
                    $this->load->model('config/equipment_config');
                    $parameter = [
                        'id' => intval($id),
                    ];
                    $result = $this->equipment_config->read($parameter);
                    if (!empty($result)) {
                        $result = $result[0];
                        $result['grade'] = 0;
                        ...
                        if ($rand <= $monster['gold_drop']) {
                            ...
                            if ($rand <= $monster['purple_drop']) //两个魔法前缀和一个魔法后缀、一个魔法前缀和两个魔法后缀
                            {
                                $result['grade'] = 3;
                                $result = $this->_generate_magic_word($monster, $role, $result);
                            } else {
                                if ($rand <= $monster['green_drop']) //一个魔法前缀和一个魔法后缀
                                {
                                    $result['grade'] = 2;
                                    $result = $this->_generate_magic_word($monster, $role, $result);
                                } else {
                                    if ($rand <= $monster['blue_drop']) //一个魔法前缀或一个魔法后缀
                                    {
                                        $result['grade'] = 1;
                                        $result = $this->_generate_magic_word(
                                            $monster,
                                            $role,
                                            $result
                                        );
                        ...
                        $parameter = [
                            ...
                            'magic_words'     => empty($result['magic_word'])
                                ? ''
                                : json_encode(
                                    $result['magic_word']
                                ),
        ...
        if ($rand <= ($monster['item_drop']) * $rate_inc) {
            if (!empty($monster['items']) && is_array($monster['items'])) {
                ...
                if (!empty($id)) {
                    ...
                    $result = $this->item_config->read($parameter);
                    if (!empty($result)) {
                        $result = $result[0];
                        ...
                        if (!empty($tmp)) {
                            $sql
                                = "UPDATE `items` SET `count`=`count`+1 WHERE `id`={$id} AND `role_id`={$role['id']}";
    ...
    {
        $pre_words_groups = [1, 2, 3, 4, 5, 6, 7];
        $next_words_groups = [1, 2, 3, 4, 5];
        ...
        for ($i = 0; $i < $item['grade']; $i++) {
            ...
            $this->load->model('config/magic_word_config');
            $parameter = [
                'type'  => $type,
                'group' => $group,
            ];
            $word = $this->magic_word_config->read(
                $parameter,
                [
                    'where_lt'     => ['level', $monster['level'] + 2],
                    'where_not_in' => ['id', $not_in_id],
                    'order_by'     => ['level', 'desc'],
                ],
                1
            );
