...
class RoleAdapter
{
    ...
    private function initialization($id)
    {
        if (!empty($id)) {
            ...
            if (!empty($role) && !empty($thirdpart)) {
                ...
                $this->role['skill'] = empty($this->role['skill'])
                    ? []
                    : json_decode(
                        $this->role['skill'],
                        true
                    );
                $this->role['passive_skill'] = empty($this->role['passive_skill']) ? []
                    : json_decode($this->role['passive_skill'], true);
                $this->role['gift'] = empty($this->role['gift'])
                    ? []
                    : json_decode(
                        $this->role['gift'],
                        true
                    );
                $this->role['append_status'] = empty($this->role['append_status']) ? []
                    : json_decode($this->role['append_status'], true);
    ...
    public function check_role_status($enforce = false)
    {
        if (!empty($this->role['append_status'])) {
            ...
            if ($changed || $enforce) {
                $this->CI->load->model('config/race_config');
                $param = [
                    'id' => $this->role ['race'],
                ];
                $raceResult = $this->CI->race_config->read($param);
                $raceResult = $raceResult [0];

                $this->CI->load->model('config/job_config');
                $param = [
                    'id' => intval($this->role ['job']),
                ];
                $jobResult = $this->CI->job_config->read($param);
    ...
    public function role_level_up()
    {
        ...
        $this->CI->load->model('config/exp_config');
        $param = [
            'level' => intval($this->role ['level']),
        ];
        $expResult = $this->CI->exp_config->read($param);
    ...
    public function calculate_property($raceResult, $jobResult, $type = 'all')
    {
        if ($type == 'all' || $type == 'health') {
            $tmp_base = $this->role ['health_base'] + $this->role['level']
                                                      * $raceResult['health_inc'];
            $this->role ['health_max'] = $tmp_base; //种族加成
            $this->role ['health_max'] += ($jobResult['health_add'] + $tmp_base
                                                                      * $jobResult['health_inc']); //职业加成
        ...
        foreach ($result as $equipment) {
            if ($type == 'all' || $type == 'atk') {
                $this->role ['atk'] += ($equipment['atk_base'] + $equipment['atk_inc']
                                        + $equipment['atk_upgrade']);
            }
            if ($type == 'all' || $type == 'def') {
                $this->role ['def'] += ($equipment['def_base'] + $equipment['def_inc']
                                        + $equipment['def_upgrade']);
            }
            if ($type == 'all' || $type == 'mdef') {
                $this->role ['mdef'] += ($equipment['mdef_base'] + $equipment['mdef_inc']
                                         + $equipment['mdef_upgrade']);
            }
            if ($type == 'all' || $type == 'health') {
                $this->role ['health_max'] += ($equipment['health_max_base']
                                               + $equipment['health_max_inc']
                                               + $equipment['health_max_upgrade']);
            }
            if ($type == 'all' || $type == 'health') {
                $this->role ['health'] += ($equipment['health_max_base']
                                           + $equipment['health_max_inc']
                                           + $equipment['health_max_upgrade']);
            }
            if ($type == 'all' || $type == 'hit') {
                $this->role ['hit'] += ($equipment['hit_base'] + $equipment['hit_inc']
                                        + $equipment['hit_upgrade']);
            }
            if ($type == 'all' || $type == 'flee') {
                $this->role ['flee'] += ($equipment['flee_base'] + $equipment['flee_inc']
                                         + $equipment['flee_upgrade']);
            }
        }
