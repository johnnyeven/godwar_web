...
class OAuth
{
    ...
    public static function init()
    {
        ...
        self::$CI->load->config('tencent_weibo.config');
        $client_id = self::$CI->config->item('tencent_weibo_app_key');
        $client_secret = self::$CI->config->item('tencent_weibo_app_secret');
    ...
    public static function refreshToken()
    {
        $params = array(
            ...
            'refresh_token' => self::$CI->currentRole->thirdpart['tencent_weibo_refresh_token']
        );
        $url = self::$accessTokenURL.'?'.http_build_query($params);
        $r = Http::request($url);
        parse_str($r, $out);
        if ($out['access_token']) {//获取成功
            self::$CI->currentRole->thirdpart['tencent_weibo_token'] = $out['access_token'];
            self::$CI->currentRole->thirdpart['tencent_weibo_refresh_token'] = $out['refresh_token'];
            self::$CI->currentRole->thirdpart['tencent_weibo_expire_in'] = $out['expires_in'];
            self::$CI->currentRole->save();
            return $out;
        } else {
            return $r;
        }
...
class Tencent
{
    ...
    {
        if (isset(self::$CI->currentRole->thirdpart['tencent_weibo_token'])) {//OAuth 2.0 方式
            //鉴权参数
            $params['access_token'] = self::$CI->currentRole->thirdpart['tencent_weibo_token'];
            $params['oauth_consumer_key'] = OAuth::$client_id;
            $params['openid'] = self::$CI->currentRole->thirdpart['tencent_weibo_expire_in'];
        ...
        } elseif (isset(self::$CI->currentRole->thirdpart['tencent_weibo_id']) && isset(self::$CI->currentRole->thirdpart['tencent_weibo_key'])) {//openid & openkey方式
            $params['appid'] = OAuth::$client_id;
            $params['openid'] = self::$CI->currentRole->thirdpart['tencent_weibo_id'];
            $params['openkey'] = self::$CI->currentRole->thirdpart['tencent_weibo_key'];
            $params['clientip'] = Common::getClientIp();
            $params['reqtime'] = time();
            $params['wbversion'] = '1';
