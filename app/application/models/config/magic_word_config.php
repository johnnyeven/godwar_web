...
class Magic_word_config extends CI_Model implements ICrud
{
    ...
    public function read($parameter = null, $extension = null, $limit = 0, $offset = 0)
    {
        ...
        if (!empty($extension)) {
            if (isset($extension['where_in'])) {
                $this->configDB->where_in($extension['where_in'][0], $extension['where_in'][1]);
            }
            if (isset($extension['where_not_in'])) {
                $this->configDB->where_not_in($extension['where_not_in'][0], $extension['where_not_in'][1]);
            }
            if (isset($extension['where_lt'])) {
                $this->configDB->where_lt($extension['where_lt'][0], $extension['where_lt'][1]);
            }
            if (isset($extension['order_by'])) {
                $this->configDB->order_by($extension['order_by'][0], $extension['order_by'][1]);
            }
        }
        if ($limit == 0 && $offset == 0) {
